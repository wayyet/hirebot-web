...
<body>
  <div class="container">
    <div class="header ${allPassed ? 'passed' : 'amber'}">
      ...
      <h1 class="title">${allPassed ? '人工评估满意' : '部分场景不满意'}</h1>
    ...
    <div class="stats">
      ...
      <div class="stat"><div class="stat-value green">${passed}</div><div class="stat-label">满意场景</div></div>
      <div class="stat"><div class="stat-value red">${failed}</div><div class="stat-label">不满意场景</div></div>
      <div class="stat"><div class="stat-value ${passRate >= 100 ? 'green' : passRate >= 80 ? 'amber' : 'red'}">${passRate}%</div><div class="stat-label">满意率</div></div>
    </div>

    <div class="rate-bar">
      <div class="rate-label">满意率 ${passRate}%</div>
      <div class="rate-track"><div class="rate-fill ${passRate >= 100 ? 'green' : passRate >= 80 ? 'amber' : 'red'}" style="width: ${passRate}%"></div></div>
      <div class="rate-mark">目标 100%</div>
    </div>
    ...
    <div class="scenarios">
      ...
      <div class="scenario-card ${s.verdict}">
        <div class="scenario-head">
          ...
          <span class="scenario-badge ${s.verdict}">${s.verdict === 'passed' ? '满意' : '不满意'}</span>
        ...
        <div class="scenario-stats">
          ...
          <div class="scenario-stat"><strong class="${s.verdict === 'passed' ? 'green' : 'red'}">${s.verdict === 'passed' ? '100%' : '0%'}</strong><label>场景满意率</label></div>
    ...
    <div class="conclusion ${allPassed ? 'passed' : 'amber'}">
      ...
      <p class="conclusion-text">
        该数字员工经过人工评估，共测试 <strong>${total} 个场景</strong>，其中 <strong style="color:#16a34a">${passed} 个满意</strong>
        ${failed > 0 ? `，<strong style="color:#dc2626">${failed} 个不满意</strong>` : ''}。
        ${allPassed
          ? '所有场景均满意，数字员工具备上岗能力，建议安排上岗。'
          : '存在不满意场景，建议修改配置后重新评估，或由审核人员决定是否上岗。'}
      </p>
    </div>
    ...
    <div className="human-report-modal-content">
      <div className={`human-report-modal-header ${allPassed ? 'passed' : 'amber'}`}>
        ...
        <div className="human-report-modal-title">
          {allPassed ? '人工评估满意' : '部分场景不满意'}
        </div>
        <p>共评估 {total} 个场景，满意 {passed} 个，满意率 {passRate}%</p>
      </div>

      <div className="human-report-stats">
        <div className="human-report-stat"><span className="indigo">{total}</span><label>评估场景</label></div>
        <div className="human-report-stat"><span className="green">{passed}</span><label>满意场景</label></div>
        <div className="human-report-stat"><span className="red">{failed}</span><label>不满意场景</label></div>
        <div className="human-report-stat"><span className={passRate >= 100 ? 'green' : passRate >= 80 ? 'amber' : 'red'}>{passRate}%</span><label>满意率</label></div>
      </div>

      <div className="human-report-rate-bar">
        <div className="human-report-rate-label">满意率 {passRate}%</div>
        ...
        <div className="human-report-rate-mark"><span className="green">目标 100%</span></div>
      </div>
          ...
          {scenarios.map(s => (
            <div key={s.scenario_id} className={`human-report-scenario-card ${s.verdict}`}>
              ...
              <div className="human-report-scenario-stats">
                ...
                <div><strong className={s.verdict === 'passed' ? 'green' : 'red'}>{s.verdict === 'passed' ? '100%' : '0%'}</strong><label>场景满意率</label></div>
      ...
      <div className={`human-report-conclusion ${allPassed ? 'passed' : 'amber'}`}>
        {allPassed ? '😊' : '⚠'}
        <label>评估结论</label>
        <p>该数字员工经过人工评估，共测试 <strong>{total} 个场景</strong>，其中 <strong className="green">{passed} 个满意</strong>
        {failed > 0 && <>，<strong className="red">{failed} 个不满意</strong></>}。
        {allPassed
          ? '所有场景均满意，数字员工具备上岗能力，建议安排上岗。'
          : '存在不满意场景，建议修改配置后重新评估，或由审核人员决定是否上岗。'}
        </p>
...
Object.assign(window, { AIEvalPage, HumanEvalPage, ReviewPage });/* global window, React */
