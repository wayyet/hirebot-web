...
**五件套定义**：每个数字员工实例由以下 5 个组件构成：

| 组件 | 说明 |
|---|---|
| **manifest** | 数字员工身份档案：名称、使命、负责范围、自主权级别、绝不能事项 |
| **skill** | 能力层：数字员工能做什么，每项能力的触发条件、输入输出、自主权 |
| **ontology** | 上下文层：数字员工工作所需的领域知识、业务对象、约束规则 |
| **api** | 工具层：数字员工与外部世界交互的接口规格，包含 CLI（命令行风格工具调用）、MCP（Model Context Protocol 工具）、数据库连接、RAG（检索增强）四类接入方式 |
| **config** | 配置层：运行时参数、平台凭证、环境变量等实例级配置项 |
...
sequenceDiagram
    ...
    Sandbox->>Sandbox: 加载 manifest+ontology+skills+api+config
