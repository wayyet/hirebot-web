...
SkillHub 是一种少见的中国互联网工具站设计声音 — 它不靠密集卡片网格和品牌主色边框压住页面，而是把整张画布交给一条**温暖到清冷的横向极光渐变**：左下角是奶油桃色（`#fff0e0` 附近），向右上推进经过淡粉，最终落在冰蓝紫（`#e1e9fa` 附近）。这条渐变不是 hero 的局部装饰 — 它**贯穿整页**，让卡片像漂浮在晨光里的物件，而不是钉在表格里的格子。这是它招牌的第一笔。

招牌的第二笔是**纯黑药丸按钮**。在那条柔软的暖色渐变之上，"通过对话安装""探索全部技能""+ 发布 Skill"全部用 `#000000` 实色填充，圆角拉到完全药丸（半径等于半个按钮高度）。这种"柔和背景 + 极硬黑按钮"的对比是品牌的第二招 — 把 CTA 换成品牌蓝立刻就不像 SkillHub 了。绝大多数面向开发者的中文站点会把品牌色作为 CTA 填充以求"专业感"，SkillHub 选择把品牌蓝 `#4a6cf7` 留给标题里的关键词高亮和外链，CTA 永远归于黑。
...
**关键特征：**
- 横向极光渐变贯穿整页（peach `#fff0e0` → 淡粉 → 冰蓝 `#e1e9fa`），不是局部装饰
...
- **Pure Black**（`#000000`）：所有主 CTA 按钮的实色填充、Logo 文字、最强强调文本。是对柔和渐变最强烈的反差锚点。
...
- **Pure White**（`#ffffff`）：所有卡片、安装提示框、输入框的表面色。在渐变背景上提供清晰的边界。

### 渐变与氛围

- **Cream Peach**（`#fff0e0`）：页面渐变的左下起点。带一丝奶油的暖基调。
- **Soft Pink Mid**（`#fde9eb`）：渐变中段过渡色。淡粉，几乎是滤镜效果。
- **Ice Lavender**（`#e1e9fa`）：渐变的右上终点。冰蓝带一点紫。
- **Page Gradient**：`linear-gradient(135deg, #fff0e0 0%, #fde9eb 50%, #e1e9fa 100%)` — 整页背景，不是 hero 局部。
...
- **Heading**（`#0a0a0a`）：所有大标题、卡片标题、技能名。**不是纯黑** — 留一丝温度让它在奶油背景上不刺眼。
...
奶油底色本身就是留白 — 不是空白，是"有色的空"。这让大块未填内容仍然不显单调。卡片之间的间距比卡片内容密度大得多，造成"内容岛屿"漂浮在晨光里的感觉，而不是表格。区块间 64-96px 的大间隙是品牌从容感的来源 — 比 SaaS 站常见的 32-48px 区块间距明显大一档。
...
**阴影哲学**：SkillHub 的阴影系统是**几乎不存在的**。所有阴影都用极低 alpha（0.03-0.06）的纯黑色，模糊大、扩散小、Y 偏移小。这与 Stripe 的多层蓝调阴影是相反的设计哲学 — SkillHub 不靠阴影制造层级，而靠**柔和的渐变背景 vs 纯白卡片**的色彩对比来分层。当背景本身已经是柔光的，阴影只需要存在感而不是力度。值得注意的是黑色 CTA 是唯一获得稍强阴影的元素 — `rgba(0,0,0,0.12)` 让它在悬停时产生"按下"的物理反馈，因为黑色按钮在浅色渐变背景上需要被锚住。

### 装饰性层深

- 整页渐变背景本身是一种"虚拟深度" — 左下暖、右上冷，造成对角线方向的氛围层次
...
- 使用 `linear-gradient(135deg, #fff0e0 0%, #fde9eb 50%, #e1e9fa 100%)` 作为整页 `body` 背景，不只是 hero 区
...
- Background gradient: `linear-gradient(135deg, #fff0e0 0%, #fde9eb 50%, #e1e9fa 100%)`
...
1. Always apply the page-wide gradient `linear-gradient(135deg, #fff0e0 0%, #fde9eb 50%, #e1e9fa 100%)` to `body`. SkillHub is unrecognizable without it.
2. Primary CTA is always pure black (`#000000`) with `border-radius: 999px`. Never use the brand blue for CTA fills.
3. Headings use `#0a0a0a`, not `#000000`. The 6% lift adds warmth on the cream background.
...
> **暗色模式说明**：源站当前未提供暗色模式。`preview-dark.html` 中的暗色令牌为合理推导：渐变改为 `linear-gradient(135deg, #1a0f08 0%, #1a1320 50%, #0e1224 100%)`（同色相暗化），表面 `#171717`，文字 `#fafafa`/`#a3a3a3`，**主 CTA 反转为白色药丸**（黑色在暗背景上会消失，必须反转），品牌蓝维持 `#4a6cf7` 作为高亮（在暗背景上仍有足够对比度）。
